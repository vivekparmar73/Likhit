// @ts-nocheck
import { LikhitConfig } from './types';

class ConfigManager {
  private static instance: ConfigManager;
  private config: LikhitConfig | null = null;
  ...
  public initialize(config: LikhitConfig) {
    if (this.config) {
  ...
  private createDefaultConfig(): LikhitConfig {
  ...
  public updateConfig(updates: Partial<LikhitConfig>) {
    const config = this.getConfig();
...
export const createConfig = (options: CreateConfigOptions = {}): LikhitConfig => {
  let authConfig;
